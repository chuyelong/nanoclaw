...
**Docker mount syntax:** Both read-write and readonly mounts use `-v host:container` (with `:ro` suffix for readonly).
...
1. **Ensures Docker daemon is running** - Checks and kills orphaned NanoClaw containers from previous runs
